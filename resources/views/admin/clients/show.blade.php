        ...
        <div class="bg-white border border-neutral-200 rounded-xl shadow-sm p-7">
            ...
            <dl class="text-sm space-y-3">
                ...
                @if($client->business_description)
                    <dt class="text-xs text-neutral-400 mt-3">Description</dt>
                    <dd class="text-neutral-600">{{ $client->business_description }}</dd>
                @endif
                @if($client->tone_of_voice)
                    <dt class="text-xs text-neutral-400 mt-3">Tone of Voice</dt>
        ...
        <div class="bg-white border border-neutral-200 rounded-xl shadow-sm p-7">
            ...
            <dl class="text-sm space-y-3">
                @foreach(['primary_keywords' => 'Primary Keywords', 'secondary_keywords' => 'Secondary Keywords', 'target_locations' => 'Locations', 'target_audience' => 'Audience'] as $field => $label)
                    @if($client->$field)
                        <dt class="text-xs text-neutral-400 mt-3">{{ $label }}</dt>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-neutral-100 bg-neutral-50">
                        ...
                        <th class="text-left px-6 py-4 text-xs font-semibold text-neutral-500 uppercase tracking-wide">Blogs</th>
                ...
                <tbody class="divide-y divide-neutral-100">
                    @foreach($client->jobs as $job)
                        <tr class="hover:bg-neutral-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-neutral-600">{{ $job->blogs()->count() }}</td>
